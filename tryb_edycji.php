...
function wyswietlam($tabela, $row){  $id = $row['id'];
   echo '<div class="arkusz">';
   echo '
         <div class="edytuj">
         <form method="post" action="tryb_edycji.php?zestaw='.$tabela.'&'.$tabela.'=edytuj&id='.$row['id'].'">
         <input type="text" name="a'.$id.'" placeholder="Wyrażenie polskie:" value="'.$row['angielskie'].'">
         <input type="text" name="b'.$id.'" placeholder="Wrażenie angielskie:"  value="'.$row['polskie'].'">
    ...
    try
    {
        ...
        if ($row['flaga'] == 1){
                echo '
                   ...
                   <div class="wybrany_zestaw_zielony"><h2>'.$tabela.'(AKTYWNY)</h2>
                     </div>
                ...
                echo '
                   ...
                   <div class="wybrany_zestaw_czerwony"><h2>'.$tabela.'(NIEKATYWNY)</h2>
                     </div>
...
function dodaj_do_planu_nauki($tabela){
        echo '
                   <div><center><h2>DODAWANIE SŁÓWEK DO PLANU NAUKI</h2></center>
                   <center><button id="dodaj_slowka_do_planu_nauki" class="przycisk przycisk1">Dodaj wszystkie</button></center>
                   </div>';
...
;
            ...
            echo "<center><h1>DODAWANIE SŁÓWEK DO ZESTAWU</h1></center>";
            formularz_dodaj($tabela);
            dodaj_do_planu_nauki($tabela);
echo '
           </div>
   </div>
</div>'
...
case 'dodaj':
        ...
        try
        {
             $polecenie = "INSERT INTO $tabela ($angielski, $polski, $zdanie, $flaga_slowa ) VALUES ('$dodaj_element1', '$dodaj_element2', '$dodaj_element3', '0')";
...
$edytuj_element3 = filtruj($_POST['c'.$id.'']);
     ...
     try
     {
          $polecenie = ("UPDATE $tabela SET  angielskie = '$edytuj_element1' , polskie = '$edytuj_element2', zdanie = '$edytuj_element3', flaga_slowa = '1' WHERE id = '$id' ");
          echo $polecenie;
          $liczba = $pdo ->exec($polecenie);
          header("Location: tryb_edycji.php?zestaw=$tabela");
...
case 'dodaj_slowka_do_planu_nauki':

    try
    {
          $liczba = $pdo -> query("UPDATE $tabela SET  $flaga_slowa = '1'");
...
$(document).ready(function(){
  $('#dodaj_slowka_do_planu_nauki').click(function(){
    let queryString = window.location.search;
    let urlParams = new URLSearchParams(queryString);
    let zestaw = urlParams.get('zestaw');
    window.document.location.href="tryb_edycji.php?zestaw="+zestaw+"&"+zestaw+"=dodaj_slowka_do_planu_nauki";
    return false;
  });
});
